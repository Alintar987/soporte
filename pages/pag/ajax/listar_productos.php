...
if($action == 'ajax'){
	...
	$tables2="visitas";
	$tables3="equipos";
	$onw="ordenes_servicio.id_caso=visitas.caso";
	$onw1="ordenes_servicio.id_serial_equipos=equipos.id_equipo";
	$campos="*";
	$sWhere="visitas.work_order LIKE '%".$query."%' OR ordenes_servicio.nombre_clientes LIKE '%".$query."%' OR ordenes_servicio.tipo_servicio LIKE '%".$query."%' OR ordenes_servicio.num_caso LIKE '%".$query."%' OR ordenes_servicio.id_caso LIKE '%".$query."%' OR ordenes_servicio.estado LIKE '%".$query."%' OR ordenes_servicio.ciudad LIKE '%".$query."%' OR equipos.serial_equipo LIKE '%".$query."%'";
	$sWhere2=" OR visitas.work_order LIKE '%".$query."%'";
	...
	$count_query   = mysqli_query($con,"SELECT count(*) AS numrows FROM ($tables INNER JOIN $tables2 ON $onw) INNER JOIN $tables3 ON $onw1 where $sWhere ");
	if ($row= mysqli_fetch_array($count_query)){$numrows = $row['numrows'];}
	else {echo mysqli_error($con);}
	$total_pages = ceil($numrows/$per_page);
	//main query to fetch the data
	$query = mysqli_query($con,"SELECT $campos FROM  ($tables INNER JOIN $tables2 ON $onw) INNER JOIN $tables3 ON $onw1 where $sWhere LIMIT $offset,$per_page");
